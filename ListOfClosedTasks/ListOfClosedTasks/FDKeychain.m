	...
	error: (NSError **)error
{
    ...
    //NSLog(@"3 - raw return %@ error - %d", rawData, error);
	return rawData;
}
	...
	error: (NSError **)error
{
    ...
    //NSLog(@"2 - itemForKey return %@ error - %d", item, error);
	return item;
}
	...
	error: (NSError **)error
{
    ...
    //NSLog(@"1 - itemForKey return %@ error - %d", item, error);
	return item;
}
	...
	error: (NSError **)error
{
	...
	else
	{
		...
		NSDictionary *itemFromKeychain = [self _itemAttributesAndDataForKey: key 
			forService: service 
			inAccessGroup: accessGroup 
			error: nil]; // &itemFromKeychainError
        
		// If any error except "Item Not Found" occured when checking if the item existed immediately fail out.
		if (itemFromKeychain == nil && [itemFromKeychainError code] != errSecItemNotFound)
		{
